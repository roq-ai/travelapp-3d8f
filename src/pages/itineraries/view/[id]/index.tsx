...
import { getItineraryById } from 'apiSdk/itineraries';
...
import { deleteTodoById } from 'apiSdk/todos';
...
function ItineraryViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ItineraryInterface>(
    ...
    () =>
      getItineraryById(id, {
        relations: ['organization', 'budget', 'todo'],
      }),
  );
  ...
  const todoHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('itinerary', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/itineraries/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('budget', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/budgets/create?itinerary_id=${data?.id}`}>
              ...
              {hasAccess('todo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/todos/create?itinerary_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.todo?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/todos/view/${record.id}`)} key={record.id}>
                            <Td>{record.task}</Td>
                            <Td>
