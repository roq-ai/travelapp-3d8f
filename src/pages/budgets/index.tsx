...
import { getBudgets, deleteBudgetById } from 'apiSdk/budgets';
...
function BudgetListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('budget', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/budgets/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('budget', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/budgets/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('budget', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/budgets/edit/${record.id}`} passHref legacyBehavior>
                          <Button
